...
| **New version deployed** | The What's New changelog for the release — title, summary, and every change — posted a few minutes after a new build boots. |
...
## New-version-deployed notification

A few minutes after boot (default 3), the backend posts the What's New entry for the running release, then never re-posts it — it is **deduped by release version**, so a pod restart or a patch rebuild of the same release stays silent and only a genuinely new release announces.

- The changelog comes from the dashboard's own **What's New** source — the backend fetches it from `${FRONTEND_URL}/whats-new.json`, so there's no second copy to keep in sync. The short delay also lets the dashboard finish coming up.
- Only real `major.minor.patch` builds announce; local/`dev` builds stay silent.
- If the running release has no What's New entry, nothing is posted (every release is expected to ship with one).

<Note>
  No extra configuration — this reuses `FRONTEND_URL` and the build's `APP_VERSION`. It's on whenever Slack is enabled.
</Note>
