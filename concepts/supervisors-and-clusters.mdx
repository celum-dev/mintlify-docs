...
<Frame caption="A supervisor's Summary tab — node, CPU, memory and storage rollups, and the tenant clusters provisioned on it.">
  ![Celum supervisor overview showing node, CPU, memory and storage tiles above a table of tenant clusters](/images/supervisor-overview.png)
</Frame>
...
<Frame caption="The Infrastructure tab — the supervisor's own hosts. Each row drills into per-host hardware, network, storage, and sensors.">
  ![Celum supervisor Infrastructure tab listing three control-plane hosts with CPU, memory, disk and NIC metrics](/images/supervisor-infrastructure.png)
</Frame>
